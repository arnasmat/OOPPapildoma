...
void link_apdorojimas(std::vector<std::wstring>& links, std:: wstringstream& eilute) {
    ...
    //cia kodas biski goofy, biski mano sizofrenijos, biski chatuko nesamoniu bet veikia!
    std::wstring zodis;
    while (eilute >> zodis) {
        if (zodis.find(L'.') != std::wstring::npos) {
            ...
            //cia bsk del edge case pridejau, pvz vu.lt/, pavercia i vu.lt/
            if (zodis.back() == ',' || zodis.back() == ';' || zodis.back() =='.') {zodis.pop_back();}
